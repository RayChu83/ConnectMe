        ...
        <section className="post-content">
          ...
          <section className="post--interactions">
            {props.likes?.includes(loggedInUser?.userId) 
              ? 
                <button className='unstyled--btn pointer liked normal--fontsize no-padding' onClick={unlike}><i class="fa-solid fa-thumbs-up liked"></i> {props.likes?.length || 0} Like{props.likes?.length !== 1 && "s"}</button> 
              : 
                <button className='unstyled--btn pointer understated normal--fontsize no-padding' onClick={like}><i class="fa-solid fa-thumbs-up understated"></i> {props.likes?.length || 0} Like{props.likes?.length !== 1 && "s"}</button>
            }
            <Link to={`/post/${props.postId}`}><small className='text--cta pointer normal--fontsize'>More Details</small></Link>
          </section>
      </article>
    </>
  )
