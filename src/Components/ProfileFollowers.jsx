...
export default function ProfileFollowers() {
  ...
  return (
    <>
      ...
      {usersTotalFollowers > 5 
        ? 
          <p className='understated pointer limit--posts' onClick={toggleShowAllPosts}>{isShowingAll ? "Show Less" : "Show All"}</p> 
        :
          usersTotalFollowers === 0 && <p className='understated text--center'>{userId === loggedInUser.userId ? <>No followers yet? Begin by <Link to="/" className='text--cta'>Creating Posts!</Link> </> : "This user currently has no followers yet."}</p>
      }
    </>
  )
}
